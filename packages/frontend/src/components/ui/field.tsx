...
function FieldDescription({ className, ...props }: React.ComponentProps<'p'>) {
  return <p className={cn('mt-1 text-xs font-normal text-ink-500', className)} {...props} />;
}

function FieldLabel({ className, ...props }: React.ComponentProps<'label'>) {
  return <label className={cn('block cursor-pointer text-sm', className)} {...props} />;
}

function FieldTitle({ className, ...props }: React.ComponentProps<'span'>) {
  return <span className={cn('block text-sm font-semibold text-ink-900', className)} {...props} />;
}
