...
type RepoServiceShape = {
  ...
  getProviderStatuses(): Effect.Effect<
...
class RepoService extends Effect.Tag('RepoService')<
  RepoService,
  RepoServiceShape
>() {}
...
const makeRepoService = Effect.gen(function* () {
  ...
  const listProviderAccounts: RepoServiceShape['listProviderAccounts'] =
    Effect.fn('RepoService.listProviderAccounts')(function* () {
      return yield* tokenStore.listAccounts();
    });

  const getProviderStatuses: RepoServiceShape['getProviderStatuses'] =
    Effect.fn('RepoService.getProviderStatuses')(function* () {
  ...
  const listNamespaceRepos: RepoServiceShape['listNamespaceRepos'] = Effect.fn(
    'RepoService.listNamespaceRepos',
  )(function* (accountId, namespacePath, limit = 100) {
    const provider = yield* providers.forAccount(accountId);
    return yield* provider.listNamespaceRepos(namespacePath, limit);
  });
